...
[ExecuteInEditMode]
public class EffectMaskUVAlphaBlendUtility : MonoBehaviour {
	...
	void Update () {
        ...
        else
        {
            ...
            if (filter.sharedMesh == null || render.sharedMaterial == null) return;
            if (render.sharedMaterial.shader.name != "Effect/Mask_UV_AlphaBlend_Offseting")
            {
                UnityEngine.Debug.LogError(name + "====?EffectMaskUVAlphaBlendUtility 必须使用这个shader：  Effect/Mask_UV_AlphaBlend_Offseting ");
                return;
            }
    ...
    void OnEnable()
    {
        ...
        if (!Application.isPlaying)
        {
            var thi = GetComponents<EffectMaskUVAlphaBlendUtility>();
