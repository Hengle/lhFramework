...
[ExecuteInEditMode]
public class EffectMask2UVAdditiveUtility : MonoBehaviour {
	...
	void Update () {
        ...
        else
        {
            ...
            if (filter.sharedMesh == null || render.sharedMaterial == null) return;
            if (render.sharedMaterial.shader.name != "Effect/Mask2_UV_Additive_Offseting")
            {
                UnityEngine.Debug.LogError(name + "====?EffectMask2UVAdditiveUtility 必须使用这个shader：  Effect/Mask2_UV_Additive_Offseting ");
                return;
            }
            ...
            //Vector4 mask3 = Vector4.zero;
            //Vector4 mask2 = Vector4.zero;
            //for (int i = 0; i < uv3.Count; i++)
            //{
            //    var x = (int)(uv3[i].x / 100);
            //    var y = (int)(uv3[i].y / 100);
            //    var z = (int)(uv3[i].z / 100);
            //    var w = (int)(uv3[i].w / 100);
            //    vec = new Vector4(x, y, z, w);
            //    mask3= uv3[i] - vec*100;
            //}
            //for (int i = 0; i < uv2.Count; i++)
            //{
            //    var x = (int)(uv2[i].x / 100);
            //    var y = (int)(uv2[i].y / 100);
            //    var z = (int)(uv2[i].z / 100);
            //    var w = (int)(uv2[i].w / 100);
    ...
    void OnEnable()
    {
        ...
        if (!Application.isPlaying)
        {
            var thi = GetComponents<EffectMask2UVAdditiveUtility>();
