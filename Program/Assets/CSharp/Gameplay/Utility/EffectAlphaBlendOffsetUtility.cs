...
[ExecuteInEditMode]
public class EffectAlphaBlendOffsetUtility : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (filter.sharedMesh == null || render.sharedMaterial == null) return;
            if (render.sharedMaterial.shader.name!="Effect/AlphaBlend_Offseting")
            {
                UnityEngine.Debug.LogError(name+ "====?EffectAlphaBlendOffsetUtility 必须使用这个shader：  Effect/AlphaBlend_Offseting ");
                return;
            }
    ...
    void OnEnable()
    {
        ...
        if (!Application.isPlaying)
        {
            var thi = GetComponents<EffectAlphaBlendOffsetUtility>();
