...
namespace lhFramework.Tools.Pools
{
    ...
    public class ModelRuntimeWindow :EditorWindow
    {
        ...
        private List<Dictionary<int, ModelPool>> m_runtimeData = new List<Dictionary<int, ModelPool>>();
        ...
        private List<ModelPool> pool = new List<ModelPool>();
        private List<Dictionary<int, ModelPool>> dicPool = new List<Dictionary<int, ModelPool>>();
        private static ModelRuntimeWindow m_instance;
        [MenuItem("Tools/FastifyPool/ModelRuntime")]
        static void Init()
        {
            m_instance = ModelRuntimeWindow.GetWindow<ModelRuntimeWindow>("ModelRuntimeWindow");
        ...
        void Initialize()
        {
            ModelManager.getHandler = OnGet;
            ModelManager.storeHandler = OnStore;
            ModelManager.freeHandler = OnFree;
            ModelManager.clearHandler = OnClear;
        }
        void OnDestroy()
        {
            ModelManager.getHandler = null;
            ModelManager.storeHandler = null;
            ModelManager.freeHandler = null;
            ModelManager.clearHandler = null;
        }
        void OnGUI()
        {
        ...
        {
            Dictionary<int, ModelPool> dic = GetDicPool();
            for (int j = 0; j < ModelManager.source.Length; j++)
            {
                var s = ModelManager.source[j];
        ...
        Dictionary<int,ModelPool> GetDicPool()
        {
            if (dicPool.Count > 0)
            {
                var p = dicPool[0];
            ...
            {
                return new Dictionary<int, ModelPool>();
            }
        }
        void FreeDicPool(Dictionary<int, ModelPool> p)
        {
