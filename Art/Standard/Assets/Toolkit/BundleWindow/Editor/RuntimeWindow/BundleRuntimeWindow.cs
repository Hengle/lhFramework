...
namespace Framework.Tools.Bundle
{
    ...
    public class BundleRuntimeWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(0, position.height - 198+40, position.width, 1), "", EditorStyles.textField);
            RenderCommand();
        }
        void RenderSourceData(Dictionary<int, SourceData> sourceDic)
        {
            ...
            m_waitScrollPos = EditorGUILayout.BeginScrollView(m_waitScrollPos, false,true, GUILayout.MinHeight(windowHeight-180),GUILayout.MinWidth(windowWidth-300), GUILayout.ExpandHeight(false));
            {
                EditorGUILayout.BeginVertical(GUILayout.Height(m_runtimeData.Count*20));
                {
                    GUILayout.Label("");
        ...
        void RenderParam()
        {
            m_paramScrollPos = EditorGUILayout.BeginScrollView(m_paramScrollPos);
            {
                if (m_runtimeData.Count> m_selectedCommand)
                {
                    if (m_runtimeData[m_selectedCommand].ContainsKey(m_selectedData))
                    {
                        ...
                        GUILayout.Label("", EditorStyles.textField, GUILayout.Width(300), GUILayout.Height(1));
                        for (int i = 0; i < data.dependencieds.Count; i++)
                        {
                            GUILayout.Label("  "+data.dependencieds[i].ToString());
                            //if (Event.current.type == EventType.MouseUp)
                            //{
                            //    if (new Rect(position.width - 200, 20, 300, 20).Contains(Event.current.mousePosition))
                            //    {
                            //        //PingObject(data.)
                            //    }
                            //}
