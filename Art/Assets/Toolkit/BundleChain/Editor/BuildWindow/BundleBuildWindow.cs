...
namespace lhFramework.Tools.Bundle
{
    public class BundleBuildWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/Bundle/Build Table")]
        ...
        [MenuItem("Tools/Bundle/Build Package")]
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (GUILayout.Button("Build Table", EditorStyles.toolbarButton))
                {
                    EditorUtility.DisplayProgressBar("build table", "building", 0.8f);
                            ...
                            while (categoryEnumer.MoveNext())
                            {
                                ...
                                var r = EditorGUILayout.BeginVertical();
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (new Rect(r.x, 0, categoryWidth, 20).Contains(Event.current.mousePosition))
                                            {
                                    ...
                                    GUILayout.Box(categoryEnumer.Current.Value.category, EditorStyles.helpBox, GUILayout.Width(categoryWidth));
                                    string value = (categoryEnumer.Current.Value.baseId).ToString();
                                    GUI.Label(new Rect(r.x + categoryWidth - value.Length * 9 - 10, 4, value.Length * 9, 30), value, EditorStyles.miniLabel);
                                    GUI.color = c;
                                        ...
                                        EditorGUILayout.BeginVertical(GUILayout.Height(categoryEnumer.Current.Value.scrollHeight), GUILayout.Width(categoryEnumer.Current.Value.scrollWidth * 7 + 30));
                                        {
                                            ...
                                            while (variantEnumer.MoveNext())
                                            {
                                                var re = EditorGUILayout.BeginHorizontal();
                                                {
                                                    if (Event.current.type == EventType.MouseUp)
                                                    {
                                                        if (new Rect(re.x, re.y + iFile * 20, categoryWidth, 20).Contains(Event.current.mousePosition))
                                                        {
                                                    ...
                                                    GUI.Label(new Rect(re.x+125- variantEnumer.Current.Key.Length*3, re.y + iFile * 20, categoryWidth, 20), variantEnumer.Current.Key);
                                                    GUI.Box(new Rect(re.x, re.y + iFile * 20, categoryWidth, 20), "");
                                                    GUI.color = color;
                                                }
                                                iFile++;
                                                EditorGUILayout.EndHorizontal();
